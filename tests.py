...
class TestRB(unittest.TestCase):
    ...
    def test_zeroprob_y(self):
        ...
        r1 = rb.get_rb_value(beta=.10, pY=pY1, src_cond_dists=self.src_conditionals1, num_retries=10, **self.extra_args)
        ...
        src_conditionals2 = (np.array([[0.95, 0.25,0],
                            ...
                            np.array([[0.8 , 0.4,0],
                                      [0.2 , 0.6 ,1]]))
        r2 = rb.get_rb_value(beta=.10, pY=pY2, src_cond_dists=src_conditionals2, num_retries=10, **self.extra_args)
    ...
    def test_zero_y(self):
        pY = np.array([0.0, 1.0])
        r = rb.get_rb_value(beta=.10, pY=pY, src_cond_dists=self.src_conditionals1, **self.extra_args)
    ...
    def test_AND_gate(self):
        ...
        r = rb.get_rb_value(beta=1/10, pY=pY, src_cond_dists=(srcC,srcC), num_retries=10, **self.extra_args)
    ...
    def test_int_src(self):
        pY = np.array([0, 1])
        r = rb.get_rb_value(beta=.10, pY=np.array([0.75, 0.25]), src_cond_dists=self.src_conditionals2, **self.extra_args)
        r = rb.get_rb_value(beta=.10, pY=np.array([0, 1]), src_cond_dists=self.src_conditionals2, **self.extra_args)
